...
##
##### Script aim: #####
#' Meta-analysis including redox marker as moderator per developmental stage and pollutant (METALLIC ELEMENTS)
#' 
...
# model
model6.2_tadpoles <- rma.mv(yi=lnRR, 
                            V=lnRR.sv, 
                            ...
                            sparse = F, 
                            verbose = F)
summary(model6.2_tadpoles)


# plot
plot_tadpoles <- orchard_plot(object = model6.2_tadpoles, 
                              data = df_tadpoles,
                              trunk.size = 10, 
                              branch.size = 1.2, 
                              twig.size = 0.25,
...
model6.3_adult <- rma.mv(yi=lnRR, 
                         ...
                         sparse = F, 
                         verbose = F)
summary(model6.3_adult)

# plot
plot_adult <- orchard_plot(object = model6.3_adult, 
                           data = df_adult,
                           trunk.size = 10, 
                           branch.size = 1.2, 
                           twig.size = 0.25,
...
##
##
metallic_panel <- ggarrange(plot_adult, plot_tadpoles,
                             ncol = 1, nrow = 2)
ggsave(filename = "./plots/MODEL6_METALLIC_PANEL.png", 
       plot = metallic_panel, 
       device = "png", 
       height = 150, 
       width = 175, 
