...
##
##### Script aim: #####
#' Meta-analysis including redox marker as moderator per developmental stage and pollutant (ORGANIC ELEMENTS)
#' 
...
## embryos
df_embryos <- df00 %>% 
  filter(Pollutant_Class == "Others_organic_compound") %>% 
...
# model
model8.2_tadpoles <- rma.mv(yi=lnRR, 
                            V=lnRR.sv, 
                            ...
                            sparse = F, 
                            verbose = F)
summary(model8.2_tadpoles)


# plot
plot_tadpoles <- orchard_plot(object = model8.2_tadpoles, 
                              data = df_tadpoles,
                              trunk.size = 10, 
                              branch.size = 1.2, 
                              twig.size = 0.25,
...
model8.3_adult <- rma.mv(yi=lnRR, 
                         ...
                         sparse = F, 
                         verbose = F)
summary(model8.3_adult)

# plot
plot_adults <- orchard_plot(object = model8.3_adult, 
                              data = df_adult,
                              trunk.size = 10, 
                              branch.size = 1.2, 
                              twig.size = 0.25,
...
##
##
organic_panel <- ggarrange(plot_adults, plot_tadpoles,
                             ncol = 1, nrow = 2)
ggsave(filename = "./plots/MODEL8_ORGANIC_PANEL.png", 
       plot = organic_panel, 
       device = "png", 
       height = 150, 
       width = 175, 
